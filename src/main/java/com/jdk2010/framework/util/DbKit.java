...
public class DbKit {
	...
	public static boolean isAnnotation(Class clazz, String fdName,
	...
	public static String warpsavesql(Object entity, Map paramMap) {
		...
		for (int i = 0; i < fdNames.size(); i++) {
			...
			paramMap.put(conversionObjectAttribute(fdName),
					DbKit.getPropertieValue(fdName, entity));
			if ((i + 1) == fdNames.size()) {
	...
	public static String warpupdatesql(Object entity, Map paramMap) {
		...
		StringBuffer sql = new StringBuffer("UPDATE ").append(tableName)
				.append("  SET  ");
		StringBuffer whereSQL = new StringBuffer(" WHERE id").append("=:")
				.append("id");
		for (int i = 0; i < fdNames.size(); i++) {
			String fdName = fdNames.get(i);// 字段名称
			...
			if (DbKit.getPropertieValue(fdName, entity) != null) {
				paramMap.put(conversionObjectAttribute(fdName),
						DbKit.getPropertieValue(fdName, entity));
				sql.append(conversionObjectAttribute(fdName)).append("=:")
						.append(conversionObjectAttribute(fdName)).append(",");
			}
		}
	...
	public static void setPropertieValue(String p, Object o, Object value)
			throws Exception {
		for (Class<?> clazz = o.getClass(); clazz != Object.class; clazz = clazz
				.getSuperclass()) {
			try {
