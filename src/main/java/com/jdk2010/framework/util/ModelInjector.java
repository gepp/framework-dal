...
public class ModelInjector {
	...
	public static <T> T inject(Class<?> modelClass, HttpServletRequest request, boolean skipConvertError) throws Exception {
		String modelName = modelClass.getSimpleName();
		return (T)inject(modelClass, StringUtils.firstCharToLowerCase(modelName), request, skipConvertError);
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final <T> T inject(Class<?> modelClass, String modelName, HttpServletRequest request, boolean skipConvertError) throws Exception {
		Object model = null;
		try {
	...
	private static final void injectActiveRecordModel(Model<?> model, String modelName, HttpServletRequest request, boolean skipConvertError) throws Exception {
	...
	private static final Class getFieldClass(Model<?> model,String inField) throws Exception{
		...
		try{
			...
			clazz=null;
			System.out.println("请检查字段:"+inField+"在：model"+model.getClass().getName()+"是否存在");
		}
		clazz=field.getType();
		return clazz;
	}
