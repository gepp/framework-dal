...
public abstract class DefaultShardingDalClient implements DalClient,
		InitializingBean {
	...
	public <T> T queryForObject(DbKit dbKit, Class<T> clazz) {
		...
		T t = null;
		try {
			t = (T) lookupDateSourceName(dbKit.getParams()).queryForObject(dbKit.getSql(), dbKit.getParams(),BeanPropertyRowMapper.newInstance(clazz));
		} catch (EmptyResultDataAccessException e) {
			t = null;
	...
	public <T> List<T> queryForList(DbKit dbKit, Class<T> clazz) {
		logInfoSql(dbKit);
		return lookupDateSourceName(dbKit.getParams()).query(dbKit.getSql(), dbKit.getParams(),BeanPropertyRowMapper.newInstance(clazz));
	}
	...
	public Page queryForPageList(DbKit dbKit, Page page, Class clazz) {
		...
		if (pageSql == null)
			return null;
		logInfoSql(dbKit);
		page.setList(lookupDateSourceName(dbKit.getParams()).query(pageSql, dbKit.getParams(),BeanPropertyRowMapper.newInstance(clazz)));
		return page;
	...
	private Page warpPageSql(Page page, DbKit dbKit) {
		...
		String countSql = "select count(*) "
				+ DbKit.replaceFormatSqlFrom(dbKit.getSql());
